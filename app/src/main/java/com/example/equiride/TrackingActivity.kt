...
class TrackingActivity : AppCompatActivity() {
    ...
    // Kalmanovy filtry
    private val latFilter = SimpleKalmanFilter(q = 0.5, r = 20.0)
    private val lonFilter = SimpleKalmanFilter(q = 0.5, r = 20.0)
    private var kalmanInited = false
    private var fixCount = 0

    private val MIN_DIST_THRESHOLD = 0.5     // metry
    private val MAX_VALID_SPEED_KPH = 100.0  // km/h
    private var firstFix = true
    private var currentPosition: GeoPoint? = null

    // Pro aktualizaci doby jízdy každou sekundu
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // start času + spuštění updatu doby
        ...
        // BroadcastReceiver pro body ze služby
        locReceiver = object : BroadcastReceiver() {
            override fun onReceive(ctx: Context, intent: Intent) {
                if (intent.action == TrackingService.ACTION_LOCATION) {
                    ...
                    onNewLocationFiltered(lat, lon, speed)
                }
            }
        }
        registerReceiver(locReceiver, IntentFilter(TrackingService.ACTION_LOCATION))

        // FusedLocationProvider (pro inicializaci filtru)
        ...
        // požádat o GPS práva a spustit Foreground službu
        val requestPermission = registerForActivityResult(
            ActivityResultContracts.RequestPermission()
        ) { granted ->
            if (granted) startTrackingService()
            else Toast.makeText(this, "Bez GPS nelze sledovat trasu", Toast.LENGTH_LONG).show()
        }
    ...
    private fun startTrackingService() {
        Intent(this, TrackingService::class.java).also { svc ->
            svc.action = TrackingService.ACTION_START
            svc.putExtra(TrackingService.EXTRA_HORSE_ID, horseId)
    ...
    private fun onNewLocationFiltered(lat: Double, lon: Double, speedMps: Double) {
        ...
        // filtr jitteru a přehnané rychlosti
    ...
    private fun drawSegments() {
        ...
        segments.zipWithNext { (p0, _), (p1, s1) ->
            ...
            val color = when {
                ...
                kph < horse.walkSpeed       -> 0xFF006400.toInt()  // tmavě zelená
                kph < horse.trotSpeed       -> 0xFFB59F00.toInt()  // tmavší žlutá
    ...
    private fun updateStats(latestSpeedMps: Double) {
        ...
        binding.tvStats.text =
            "Body: ${segments.size}  Vzdálenost: ${"%.1f".format(dist)} m"
    ...
    private fun stopAndSave() {
        ...
        Toast.makeText(
            this,
            "Jízda uložena: ${"%.1f".format(totalDist)} m, Trvání: ${String.format("%02d:%02d", durationSec/60, durationSec%60)}",
            Toast.LENGTH_LONG
        ).show()
